...
function isLogin(cb) {
    if (!token) { //获取不到token
        if (isWX_Allipay() == 'WX') { //如果是微信客户端
            if (getUrlParam('code') == '') {
                location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3f92587de79b0592&redirect_uri=' + location.href + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
            } else {
                dlc_request('App/public/api/', { api_name: 'wx_auth', code: getUrlParam('code') }, function(res) {
                    if (res.code == 1) {
...
function emptyTip(tip) {
    return '<div class="h20"></div><div class="empty" style="display: flex;align-items: center;"><div class="logo"><img src="../img/y_empty_page_nothing.png"></div><div class="msg" style="color: #999;">' + tip + '</div></div>';
}
//判断身份证格式是否正确
...
function dlcUrl() {
    return 'http://xinfengji.app.xiaozhuschool.com/';
}
